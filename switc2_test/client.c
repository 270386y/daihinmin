  ...
  while (whole_gameend_flag == 0) {
    ...
    while (one_gameend_flag == 0) { //1ゲームが終わるまでの繰り返し
      ...
      if (receiveCards(own_cards_buf) == 1) { //カードをown_cards_bufに受け取り
        ...
        int target_id = -1; // ターゲットのID
        int my_rank_now = ss.old_rank[my_playernum]; // 自分の順位

        // 1つ上の順位の人を探す
        for(i = 0; i < 5; i++){
            if(ss.old_rank[i] == my_rank_now - 1){
                target_id = i;
                break;
            }
        }

        //fprintf(stderr, " %d",target_id);

        fprintf(stderr, "[DEBUG] My:%d, Target:%d, rank:[%d,%d,%d,%d,%d]\n", 
        my_rank_now, 
        target_id,
        ss.old_rank[my_playernum], ss.old_rank[1], ss.old_rank[2], ss.old_rank[3], ss.old_rank[4]);

        // 場にカードが出ていて(onset==0)、かつ 直前に出したのがターゲットなら
        if (state.onset == 0 && last_playernum == target_id && target_id != -1) {
            // 評価値を上げて、勝てるカードがあれば無理やり出させる
            attack_flag = 1;

            for (j = 0; j < 3; j++) {     // j:進行度(序盤中盤終盤)
                for (k = 0; k < 4; k++) { // k:役の種類
                     tes[j][k][2] += 100; 
                }
            }
        }
