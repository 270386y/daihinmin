...
//////VV-8/////////////////////
  ...
  //提出されたカード、提出されたカード+手札の集合、まだ出されていないカードを入れる	
  int submitted_cards[8][15];
  int submitted_cards_plus_hands[8][15];
  int rest_cards[8][15];

  //都合により、自分の手札をコピー
  //int temoti_cards[8][15];
	
  //状態を記録(計算時間の短縮のため）
  /*
    int rest_cards_b1[8][15];
    int rest_cards_b2[8][15];
    int rest_cards_b3[8][15];
    int rest_cards_b4[8][15];
    int rest_cards_b5[8][15];
    int rest_cards_b6[8][15];
    int rest_cards_b7[8][15];
	
    int my_cards_b1[8][15];
    int my_cards_b2[8][15];
    int my_cards_b3[8][15];
    int my_cards_b4[8][15];
    int my_cards_b5[8][15];
    int my_cards_b6[8][15];
    int my_cards_b7[8][15];
	
    int rest_cards_j1[8][15];
    int rest_cards_j2[8][15];
    int rest_cards_j3[8][15];
	
    int my_cards_j1[8][15];
    int my_cards_j2[8][15];
    int my_cards_j3[8][15];
  ...
  */
  ...
  //手札を分割し作成した組を収納（手札交換の際に、一時的に手札が13枚になることもあるため、13個作成。
  int kumi0[8][15];
  int kumi1[8][15];
  int kumi2[8][15];
  int kumi3[8][15];
  ...
  int kumi9[8][15];
  int kumi10[8][15];
  int kumi11[8][15];//手札交換時の手札組数に対応
  int kumi12[8][15];

  //手札の組み分け後に、情報を収納	
  int kumi_info[16][60];	
  //[0][]から[12][]までは組の情報//手札交換の際に手札が13枚となり組が13組になることがあるため
  //[][0]に組の種類(シングル1、ペア2、階段3)、
  //[][1]に枚数、[][2]にマーク、[][3]に組内の最弱カードのランク、[][4]に組内の最強カードのランク
  //[][5]に強さ評価値、[][6]に1枚当たりの強さ評価値、[][7に革命時の強さ評価値、
  //[][8]に1枚あたりの革命時の強さ評価値、[][9]に流し評価値、[][10]に1枚当たりの流し評価値、
  //[][11]に革命時の流し評価値、[][12]に1枚当たりの革命時の流し評価値、[][13]にJOKERの有無
  //[][14]に革命を引き起こすか
  //[][15]組内の最弱カードの値を、手札の組数によって調整したもの
  //[][15]組内の最強カードの値を、手札の組数によって調整したもの
  //が、収納されている。
  //強さ評価値と流し評価値は、8が絡んだ際に異なる数字となる。8が絡まない場合は強さ評価値と流し評価値は一致する。
	
  //[13][]は手札の情報
  //[][0]に組の数、[][1]に革命を引き起こせる組を持つか、
  //[][2]に手札評価値（強さ評価値の合計）、[][3]に革命時の手札評価値、
  //[][4]に流し評価値の合計、[][5]に革命時の流し評価値の合計
  //[][6]に規定値を超えた流し評価値（1枚当たり）を持つ組の数
  //[][7]に革命時の、規定値を超えた流し評価値（1枚当たり）を持つ組の数
  //[][8]に手札の枚数、[][9]シングルの組数[][10]ペア（2枚組）の組数
  //[][11]ペア（3枚組）の組数[][12]階段（3枚組）の組数
  //[][13]ペア（4枚組以上）の組数[][14]階段（4枚組以上）の組数
  //[][15]に「1枚当たりの流し評価値」の合計
  //[][16]に「1枚当たりの革命時の流し評価値」の合計
  //[][17]に「「1枚当たりの流し評価値」の合計」を組数で割ったもの
  //[][18]に「「1枚当たりの革命時の流し評価値の合計」を組数で割ったもの
  //[][19]に「流し評価値の合計」を組数で割ったもの
  //[][20]に「革命時の流し評価値の合計」を組数で割ったもの
  //[][21]に手札内で最も強い「シングルの組」のランク
  //[][22]に手札内で最も強い「ペア（2枚組）の組」のランク
  //[][23]に手札内で最も強い「ペア（3枚組）の組」のランク
	
  //通常時は2以外、革命時は3以外
  //[][24]に規定値以上の流し評価値をもつシングルの組数（8以外）(2以外)
  //[][25]に規定値以上の流し評価値をもつシングルの組数（8）
  //[][26]に規定値以上の流し評価値をもつペアの組数（8以外）(2以外）
  //[][27]に規定値以上の流し評価値をもつペアの組数（8）
  //[][28]に規定値以上の流し評価値をもつ3枚組(トリプル）の組数(8以外)(2以外)
  //[][29]に規定値以上の流し評価値をもつ3枚組(トリプル）の組数(8)
  //[][30]に規定値以上の流し評価値をもつ3枚組(階段）の組数
  //[][31]に規定値以上の流し評価値をもつ4枚以上のペアの組数(8以外)(2以外)
  //[][32]に規定値以上の流し評価値をもつ4枚以上のペアの組数(8)
  //[][33]に規定値以上の流し評価値をもつ4枚以上の階段の組数
							
  //[][34]に2（革命時は3）を含む階段以外の組の数
  //[][35]に規定値－1以上の流し評価値をもつ組の数//これまでの規定値100よりも緩いものを。
							
  //[][36]に規定値-1以上の流し評価値をもつシングルの組数（8以外）(2以外)
  //[][37]に規定値-1以上の流し評価値をもつシングルの組数（8）
  //[][38]に規定値-1以上の流し評価値をもつペアの組数（8以外）(2以外）
  //[][39]に規定値-1以上の流し評価値をもつペアの組数（8）
  //[][40]に規定値-1以上の流し評価値をもつ3枚組(トリプル）の組数(8以外)(2以外)
  //[][41]に規定値-1以上の流し評価値をもつ3枚組(トリプル）の組数(8)
  //[][42]に規定値-1以上の流し評価値をもつ3枚組(階段）の組数
  //[][43]に規定値-1以上の流し評価値をもつ4枚以上のペアの組数(8以外)(2以外)
  //[][44]に規定値-1以上の流し評価値をもつ4枚以上のペアの組数(8)
  //[][45]に規定値-1以上の流し評価値をもつ4枚以上の階段の組数
							
  //[][46]に2のシングルの数（革命時は3）
  //[][47]に2のペア(2枚組)
  //[][48]に2のトリプル
							
  //[][49]に規定値－2以上の流し評価値をもつ組の数//これまでの規定値100よりも緩いものを。
  //[][50]に規定値－4以上の流し評価値をもつ組//これまでの規定値100よりも緩いものを。
						
							
  //[14][]※革命時に対応していない
  //[][0]規定値(100)未満の組の数
  //[][2]規定値未満の流し評価値の合計
  //[][4]規定値未満の1枚単位の流し評価値の合計
  //[][6]  [][4]を組数で割ったもの
  //[][8]  [][2]を組数で割ったもの
  //[][10]通常時の規定値-1以上の組と3枚組以上の組の合計。
  //[][11]通常時の規定値以上の組と3枚組以上の組の合計。
  //[][12]通常時の規定値以上の組と3枚組以上の組の合計。
  //が、収納されている。
							
  //[15][]//最も評価値の低い組を除いて、手札評価値等を求めてみる。
  //[][0]//流し評価値の合計[13][4]から、1枚当たりの流し評価値が最も低い組の値を除いたもの
  ...
  int have_s3;
  //プレイヤー関連。（あまり活用できていない・・・）
  int player_number;//名前は変だが、残りプレイヤー数を保存している。
  int player_number_point;
  int pre_rank;

  //手札分割関連
  //革命を起こすかどうか、my_lead5用
  int cause_rev;
  //終盤用
  int finish;				
  ...
  int player_info[5][3];//[0][]に自分、[1][]に自分のひとつ前に座っている人・・・
  //[][0]にプレイヤー番号、[][1]に手札枚数、[][2]に階級
  int player_info2[5][3];//[0][]に自分、[1][]に自分のひとつ前に座っている人・・・
  //[][0]にプレイヤー番号、[][1]に手札枚数、[][2]に階級
  //直前のもの(?)
  int player_info3[5][3];//[0][]に自分、[1][]に自分のひとつ前に座っている人・・・
  //[][0]にプレイヤー番号、[][1]に手札枚数、[][2]に階級
  //前の自分の番のもの(?)
							
  int playable_player;//未完成
						
  int mae_player_hands;
  int onset_flag;
...
void copyTable2(int dest_table[8][15], int org_table[8][15]);//場のカードを保存
void copyTable3(int dest_table[8][15], int org_table[8][15]);//場のカードを保存
void copyTable4(int dest_table[8][15], int org_table[8][15]);//場のカードを保存
void copyTable5(int dest_table[8][15], int org_table[8][15]);//場のカードを保存//my_lead14用
...
//枚数カウント系
int qtyOfCards2(int cards[8][15]);//ペア用
int qtyOfCards3(int cards[8][15]);//階段用
int qtyOfCards4(int cards[8][15]);//「1」のみを数える
int qtyOfCards5(int cards[8][15],int rank);//そのランクのカードの枚数を数える。
int qtyOfCards6(int cards[8][15],int mark,int rank);//引数で渡されたところにカードがあるかの確認。
int qtyOfCards7(int cards[8][15],int rank);//引数で渡されたランクよりも小さいランクの枚数
int qtyOfCards8(int cards[8][15],int rank);//引数で渡されたランクよりも大きいランクの枚数
int qtyOfCards9(int cards[8][15]);//同じランクのカード4枚の提出によって、革命が起こされるかを確認
int qtyOfCards10(int cards[8][15]);//5枚組以上の階段の提出によって、革命が起こされるかを確認
int qtyOfCards11(int cards[8][15],int mark);//自分の手札と提出されたカード（提出されていなカード）を参照し、縛りの場合、自分が流す権利を持っているかを確認する。
int qtyOfCards12(int cards[8][15],int rank,int mark);//マークを指定して、引数で渡されたランクよりも小さいランクの枚数
int qtyOfCards13(int cards[8][15],int rank,int mark);//マークを指定して、引数で渡されたランクよりも大きいランクの枚数
int qtyOfCards14(int cards[8][15],int rank);//引数で渡されたランクよりも小さいランクの枚数をマークごとに数えて、最も少なかったマークを返す//submit_cardsでの使用を想定しており、0の数を数えている。
int qtyOfCards15(int cards[8][15],int rank);//引数で渡されたランクよりも大きいランクの枚数をマークごとに数えて、最も少なかったマークを返す
...
void highSolo3(int out_cards[8][15],int my_cards[8][15],int joker_flag);//高いほうから探すが、jokerは[0][14]で出す。
	
void lowCards2(int out_cards[8][15],int my_cards[8][15],int threshold,int mark);//あるマークのthreshold以上のカードは消える。
void highCards2(int out_cards[8][15],int my_cards[8][15],int threshold,int mark);//あるマークのthreshold以下のカードは消える。
...
//他のプレイヤーの提出を考える(1)
int follow_o(int out_cards[8][15],int my_cards[8][15]);//マークに関係なく、その手札で出せるカードがあるかの確認//現在の場において
...
int follow_o_shibari(int out_cards[8][15],int my_cards[8][15]);//あるマークにおいて、その手札で出せるカードがあるかの確認//ある場において
...
int follow_o2(int out_cards[8][15],int my_cards[8][15]);//マークに関係なく、その手札で出せるカードがあるかの確認//ある場において
...
//最も小さい札を提出する。	
int follow_o3_shibari(int out_cards[8][15],int my_cards[8][15]);//あるマークにおいて、その手札で出せるカードがあるかの確認//ある場において
...
int follow_o3(int out_cards[8][15],int my_cards[8][15]);//マークに関係なく、その手札で出せるカードがあるかの確認//ある場において
...
void my_finish_follow4(int out_cards[8][15],int my_cards[8][15]);//そのカードをfollowで出した後、相手がそれを流せるかを考える
...
void my_finish_follow4_rev(int out_cards[8][15],int my_cards[8][15]);//そのカードをfollowで出した後、相手がそれを流せるかを考える
...
//１枚当たりの評価値を返す
int setvalue_single2(int cards[8][15]);//kumi_infoに触れない
...
//組としての評価値を返す。（２枚以上の組では、強さ評価値等が１００を超える）
...
//どの「bunkatsu_noprint」を使うかを判断する
int bunkatsu_all2(int table[8][15]);

//手札分割を行う。
//_aは革命を起こさないように組を作成、_cは通常、_revは３を除いて組をつくる。マークなしは２を除いて組をつくる。
int bunkatsu_noprint(int table[8][15]);//２を除いて、組をつくる//階段→ペア→単体
int bunkatsu_noprint2(int table[8][15]);						//ペア→階段→単体
	
int bunkatsu_noprint_a(int table[8][15]);//２を除いて、革命を起こさない組をつくる
int bunkatsu_noprint2_a(int table[8][15]);

int bunkatsu_noprint_c(int table[8][15]);//組をつくる
int bunkatsu_noprint2_c(int table[8][15]);
	
int bunkatsu_noprint_a_c(int table[8][15]);//革命を起こさない組をつくる
int bunkatsu_noprint2_a_c(int table[8][15]);
	
int bunkatsu_noprint_rev(int table[8][15]);//３を除いて、組をつくる
int bunkatsu_noprint2_rev(int table[8][15]);
	
int bunkatsu_noprint_a_rev(int table[8][15]);//３を除いて、革命を起こさない組をつくる
int bunkatsu_noprint2_a_rev(int table[8][15]);

//jokerの使い方を考える
int bunkatsu_pre2(int table[8][15]);
int bunkatsu_pre3(int table[8][15]);//手札交換に用いている
